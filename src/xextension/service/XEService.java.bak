 ...
 * 
 */
public class CopyOfXEService {
	private ServerSocket	server;

	public CopyOfXEService() {
		String msg = "";
	...
	public static void main(String[] args) throws Exception {
		...
		CopyOfXEService service = new CopyOfXEService();
		// check if service already started
		checkServiceAlreadyStarted(service.server.getLocalPort());

		service.startService();
	}

	private static void checkServiceAlreadyStarted(final int currentPort) {
		Thread t = new Thread(new Runnable() {
			public void run() {
				...
				System.out.println("server starts on port: " + currentPort);
			}
		});
		t.setDaemon(true);
		t.start();
